 ...
 *
 * BinaryLane::servers() goes through the manager's __call() and returns mixed; what makes
 * `BinaryLane::servers()->list()` analysable is the annotation, not the code. So an accessor added
...
final class FacadeConformanceTest extends BaseTestCase
{
    ...
    public function every_annotated_return_type_resolves_to_a_real_class(): void
    {
        ...
        $doc = (string) (new ReflectionClass(BinaryLane::class))->getDocComment();
    ...
    public function the_manager_does_not_shadow_a_client_method(): void
    {
        ...
        // BinaryLane's account endpoint and would be unreachable through the facade.
    ...
    {
        return array_values(array_diff($this->publicMethods(BinaryLaneManager::class), ['__call']));
    }
    ...
    /**
     * Annotations are written short, against the facade's own imports - including an aliased
     * one, which the core package makes necessary: `Endpoint\Account` and `Entity\Account` are
     * both return types on the client.
     */
    private function resolve(string $type): string
    {
        $source = (string) file_get_contents((string) (new ReflectionClass(BinaryLane::class))->getFileName());
